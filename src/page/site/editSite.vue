<template>
	<el-row class="rowDiv">
		<el-row class="title bot">
			<el-col :span="20">编辑逻辑站点</el-col>
		</el-row>
		<el-row style="margin:20px;">
			<span class="redSpan">*</span><span>站点名称：</span><span>{{site.siteName}}</span>
		</el-row>
		<el-row style="margin:20px;">
...
<script>
	...
	export default {
		...
		created() {
			...
			this.site.siteName = this.$route.query.siteName;
		...
		methods: {
			...
			//查询站点地区
			async initData(siteId) {
				try {
					let _this = this;
					_this.ysList = [];
					_this.hsList = [];
					const returnData = await getSingleSite({
						siteId: siteId
					});
					if(returnData.errorCode == "0") {
						if(returnData.dataBody.siteRegionList != null){
							returnData.dataBody.siteRegionList.forEach(function(item){
							let obj = {
							...
							//默认显示一个已选城市，所在地区的所有城市，并隐藏已选城市
							let ys = this.ysList[0];
							_this.getRegionlist(ys.code.substr(0,2)+"0000");
						}
